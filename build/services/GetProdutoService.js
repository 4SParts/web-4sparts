...
var _Produtos = _interopRequireDefault(require("../entities/Produtos"));
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
class GetProdutosService {
  async execute() {
    const produtoRepository = await (0, _typeorm.getRepository)(_Produtos.default).query(`
	  select
	  ...
	  where
		  ...
		  and f_enviapalm = 'S'
		  --and lista_preco.d_inicio <= now()
		  --and lista_preco.d_vencimento >= now()
		  --and estoque.i_cdlocal_estoque in (1 , 99, 3)
		  --and preco.n_precovenda <> '0'
	  group by
		  id,
		  idproduto,
		  Descrição,
	  ...
	  order by
		  dcadastro desc
     `);
    const product = produtoRepository.map(product => {
...
var _default = GetProdutosService;
